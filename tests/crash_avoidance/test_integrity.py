...
class TestRebuildTrustedInput(unittest.TestCase):
    ...
    def test_input_parameters_value_is_preserved(self):
        ...
        ws.cell(row=2, column=value_col).value = "Standard"
        user_path = self._save_wb(wb)
    ...
    def test_footnote_asterisk_on_input_parameter_is_tolerated(self):
        """A user file reflecting a newer template revision may carry a
        trailing footnote marker ("*") on an Input parameter label. That's a
        display-only detail (the footnote text itself lives in its own
        column) and must not trip the identity-column trust boundary."""
        wb = openpyxl.load_workbook(self.pristine_path)
        ws = wb["Input parameters"]
        header = [c.value for c in next(ws.iter_rows(min_row=1, max_row=1))]
        input_parameter_col = header.index("Input parameter") + 1
        original_value = ws.cell(row=2, column=input_parameter_col).value
        ws.cell(row=2, column=input_parameter_col).value = f"{original_value}*"
        user_path = self._save_wb(wb)
        ...
        self.assertEqual(
            ...
            .cell(row=2, column=input_parameter_col)
            .value,
            original_value,
        )

    def test_input_parameter_mismatch_beyond_footnote_is_still_rejected(self):
        """A genuine label change (not just a trailing footnote marker) must
        still be rejected, with a message naming the offending row/column."""
        wb = openpyxl.load_workbook(self.pristine_path)
        ws = wb["Input parameters"]
        header = [c.value for c in next(ws.iter_rows(min_row=1, max_row=1))]
        input_parameter_col = header.index("Input parameter") + 1
        ws.cell(row=2, column=input_parameter_col).value = "Prediction - Bogus"
        user_path = self._save_wb(wb)

        with self.assertRaises(common.TemplateIntegrityError) as ctx:
            integrity.rebuild_trusted_input(user_path)

        message = str(ctx.exception)
        self.assertIn("Row 2", message)
        self.assertIn("Input parameter", message)
        self.assertIn("Prediction - Bogus", message)
        self.assertIn("Prediction - Standard", message)
    ...
    def test_pred_sheet_passthrough_is_untouched(self):
        """The color-grid prediction sheet isn't schema'd -- it must pass
        through from the user's file as-is."""
        wb = openpyxl.load_workbook(self.pristine_path)
        ws = wb["FC - Car & PTW pred."]
        ...
        sanitized_wb = openpyxl.load_workbook(sanitized_path, data_only=True)
        self.assertEqual(
            sanitized_wb["FC - Car & PTW pred."]["E5"].value, "sentinel-value"
        )
...
class TestRebuildTrustedInputDfs(unittest.TestCase):
    ...
    calculate_score(dfs)/add_verification_sheets_to_dfs) instead of
    ...
    def test_input_parameters_value_is_preserved(self):
        ...
        dfs["Input parameters"].loc[0, "Value"] = "Standard"

        sanitized = integrity.rebuild_trusted_input_dfs(dfs)

        self.assertEqual(sanitized["Input parameters"].loc[0, "Value"], "Standard")

    def test_footnote_asterisk_on_input_parameter_is_tolerated(self):
        dfs = self._dfs()
        dfs["Input parameters"] = dfs["Input parameters"].astype(object)
        original_value = dfs["Input parameters"].loc[0, "Input parameter"]
        dfs["Input parameters"].loc[0, "Input parameter"] = f"{original_value}*"

        sanitized = integrity.rebuild_trusted_input_dfs(dfs)

        self.assertEqual(
            sanitized["Input parameters"].loc[0, "Input parameter"], original_value
        )

    def test_input_parameter_mismatch_beyond_footnote_is_still_rejected(self):
        dfs = self._dfs()
        dfs["Input parameters"] = dfs["Input parameters"].astype(object)
        dfs["Input parameters"].loc[0, "Input parameter"] = "Prediction - Bogus"
    ...
    def test_pred_sheet_passthrough_is_untouched(self):
        dfs = self._dfs()
        dfs["FC - Car & PTW pred."] = dfs["FC - Car & PTW pred."].astype(object)
        dfs["FC - Car & PTW pred."].iloc[2, 3] = "sentinel-value"

        sanitized = integrity.rebuild_trusted_input_dfs(dfs)

        self.assertEqual(sanitized["FC - Car & PTW pred."].iloc[2, 3], "sentinel-value")

    def test_sheet_missing_from_dfs_is_skipped_not_rejected(self):
        """calculate_score(dfs)/add_verification_sheets_to_dfs callers may
...
class TestFindMissingRequiredInputs(unittest.TestCase):
    ...
    packaged ca_template.xlsx, covering both the schema'd "Input
    parameters" sheet and the dynamic "...pred."/"...robust. pred." grid
    sheets (see common.find_missing_grid_inputs)."""
    ...
    def test_fresh_template_reports_grid_and_input_parameter_gaps(self):
        ...
        self.assertIn("FC - Car & PTW pred.", sheets)
        self.assertIn("Input parameters", sheets)

    def test_scoping_to_one_protocol_element_excludes_other_grid_sheets(self):
        scoped = integrity.find_missing_required_inputs(
            self.wb, stage_element="Frontal Collisions", stage_subelement="Car & PTW"
        )

        self.assertTrue(scoped)
        self.assertEqual(
            {m.sheet for m in scoped},
            {
                "FC - Car & PTW pred.",
    ...
    def test_driver_state_link_belongs_to_ldc_single_vehicle(self):
        """FC-prefixed, but the Driver state link loadcase sits under Lane
        Departure Collisions / Single vehicle in data_model.py -- its 45
        dropdown cells are OEM input and must be required under that
        element (and no other)."""
        single_vehicle = integrity.find_missing_required_inputs(
            self.wb,
            stage_element="Lane Departure Collisions",
            stage_subelement="Single vehicle",
        )
        frontal = integrity.find_missing_required_inputs(
            self.wb, stage_element="Frontal Collisions", stage_subelement="Car & PTW"
        )

        dsl = [m for m in single_vehicle if m.sheet == "FC - Driver State Link"]
        self.assertEqual(len(dsl), 45)
        self.assertNotIn("FC - Driver State Link", {m.sheet for m in frontal})

    def test_ldc_robust_pred_sheet_is_shared_across_two_subelements(self):
        single_vehicle = integrity.find_missing_required_inputs(
            self.wb,
            stage_element="Lane Departure Collisions",
            stage_subelement="Single vehicle",
        )
        car_and_ptw = integrity.find_missing_required_inputs(
            self.wb,
            stage_element="Lane Departure Collisions",
            stage_subelement="Car & PTW",
        )

        self.assertIn("LDC - robust. pred.", {m.sheet for m in single_vehicle})
        self.assertIn("LDC - robust. pred.", {m.sheet for m in car_and_ptw})

    def test_filling_grid_cell_with_text_removes_it_from_missing(self):
        ws = self.wb["LDC - Single Veh pred."]
    ...
    def test_filling_grid_cell_with_color_only_removes_it_from_missing(self):
        """Post-preprocess, report_writer clears the OEM's typed text and
        encodes the answer as a fill color instead -- the check must
        recognize that shape too, not just literal text."""
    ...
    def test_descriptions_are_semantic_for_grid_sheets(self):
        missing = integrity.find_missing_required_inputs(
            self.wb,
            stage_element="Lane Departure Collisions",
            stage_subelement="Single vehicle",
        )
        grid_entries = [m for m in missing if m.sheet == "LDC - Single Veh pred."]

        self.assertTrue(grid_entries)
        entry = grid_entries[0]
    ...
    def test_unknown_scope_raises(self):
        ...
        with self.assertRaises(ValueError):
            integrity.find_missing_required_inputs(
                wb, stage_element="frontal_collisions", stage_subelement="car_and_ptw"
            )
        with self.assertRaises(ValueError):
...
class TestPristineTemplateRegression(unittest.TestCase):
    ...
    EXPECTED_PER_SHEET = {
        "FC - Driver State Link": 45,
        "FC - Car & PTW pred.": 473,
        "FC - Ped & Cyc pred.": 457,
        "FC - Car & PTW robust. pred.": 85,
        "FC - Ped & Cyc robust. pred.": 92,
        "LDC - Single Veh pred.": 36,
        "LDC - Car & PTW pred.": 206,
        "LDC - robust. pred.": 32,
        "LSC - Car & PTW pred.": 19,
        "LSC - Ped & Cyc pred.": 36,
        "Input parameters": 55,
    ...
    def test_version_finding_is_reported_even_when_scoped(self):
        ...
        with patch.object(common, "VERSION", "5.4.6"):
            ...
            findings = integrity.find_missing_required_inputs(
                wb, stage_element="Frontal Collisions", stage_subelement="Car & PTW"
            )
